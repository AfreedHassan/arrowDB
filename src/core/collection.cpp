...
Space Collection::space() const { return pImpl_->config_.space; }
  ...
  if (vec.size() != pImpl_->config_.dimensions) {
    return utils::Status(utils::StatusCode::kDimensionMismatch,
                         "Vector dimension mismatch: expected " +
                             std::to_string(pImpl_->config_.dimensions) +
...
utils::Status Collection::save(const std::string &directoryPath) {
  ...
  RecoveryMetadata recovery{
      ...
      .lastPersistedTxid =
          (pImpl_->txidCounter > 0) ? pImpl_->txidCounter - 1 : 0,
      .cleanShutdown = true};
...
utils::Result<Collection> Collection::load(const std::string &directoryPath) {
  ...
  CollectionConfig config{.name = internalCfg.name,
                          .dimensions = internalCfg.dimensions,
                          .space = internalCfg.space,
                          .index = {.max_elements = hnswCfg.maxElements,
                                    .M = hnswCfg.M,
                                    .ef_construction = hnswCfg.efConstruction}};

  auto impl = std::make_unique<Impl>(config, fs::path(directoryPath));
