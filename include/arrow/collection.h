 ...
 */
std::pair<CollectionConfig, HNSWConfig>
...
public:
  ...
  /// @throws std::runtime_error if directory doesn't exist, files are missing,
  /// or loading fails
  static Collection load(const std::string &directoryPath) {
    ...
    if (!fs::exists(directoryPath) || !fs::is_directory(directoryPath)) {
      throw std::runtime_error("Collection directory does not exist: " +
                               directoryPath);
    }
    ...
    if (!fs::exists(metaPath)) {
      throw std::runtime_error("meta.json not found in collection directory: " +
                               directoryPath);
    }
    ...
    if (!fs::exists(indexPath)) {
      throw std::runtime_error("index.bin not found in collection directory: " +
                               directoryPath);
    }
...
inline std::pair<CollectionConfig, HNSWConfig>
importConfigsFromJson(const std::string &filepath) {
