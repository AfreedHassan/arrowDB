...
#include "arrow/vector_store.h"
#include "arrow/flat_search.h"
...
TEST(FlatSearchTest, SimpleCosineSearch) {
    ...
    store.insert(3, validateAndNormalize({0,0,1},3));
		...
		std::vector<float> query = validateAndNormalize({1,0.5,0},3);
		std::vector<SearchResult> results = flatSearch(store, query, 2);
